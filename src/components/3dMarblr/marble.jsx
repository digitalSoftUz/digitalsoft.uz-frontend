...
import Noise from "./aaa.jpg"
// import Noise3d from "./noise3D.jpg"
// import Sphere3d from "./sphere.png"
// import Energy from "."
// HSL values
const options = [
  [1, 100, 50],
...
export default function Marble3D() {
  ...
  return (
    <aw.div style={{ background: "none", width: '100%', height: '100%' }}>
...
function Marble({ step, setStep }) {
  ...
  return (
    <group>
      <a3.group scale={scale} onClick={() => setStep(step)} transparent={0}>
        <Sphere args={[1, 64, 32]}>
          <MagicMarbleMaterial step={step} />
        </Sphere>
      </a3.group>
...
function MagicMarbleMaterial({ step, ...props }) {
  ...
  const heightMap = useTexture(Noise)
  const displacementMap = useTexture(Noise)
  ...
  const [uniforms] = useState(() => ({
    ...
    colorB: { value: new THREE.Color(0x115164) },
    // colorB: { value: new THREE.Color(0x115164) },
